...
<b-card @click.prevent="toggleItem($event,data.id)" :class="{'d-flex flex-row':true,'active' : selectedItems.includes(data.id)}" no-body>
    <div class="pl-2 d-flex flex-grow-1 min-width-zero">
        <div class="card-body align-self-center d-flex flex-column flex-lg-row justify-content-between min-width-zero align-items-lg-center">
            ...
            <p class="mb-0 text-muted text-small w-15 w-sm-100">{{data.agent_type}}</p>
            <p class="mb-0 text-muted text-small w-15 w-sm-100">{{data.garage}}</p>
            <div class="w-15 w-sm-100">
                <b-badge pill :variant="data.statusColor">{{ data.phone }}</b-badge>
            </div>
        </div>
